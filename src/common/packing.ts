...
export class Packing {
  ...
  public async start(skipPrepare: boolean) {
          ...
          await Promise.all(
            this.dependencies.map((dependencyDirectory) => {
              return glob
                .promise(
                  "**",
                  Object.assign(globOptions, { cwd: dependencyDirectory })
                )
                .then((data) =>
                  data.map((name) => path.join(dependencyDirectory, name))
                );
            })
          )
        );
    ...
    this.toZipFiles = this.toZipFiles.concat(
      await glob
        .promise(
          "**",
          Object.assign(globOptions, { cwd: this.pluginRootFolder })
        )
        .then((data: string[]) =>
          data.map((name) => path.join(this.pluginRootFolder, name))
        )
    );
  ...
  private async doExclude() {
    ...
    const zipPath = path.resolve(
      this.pluginRootFolder + path.sep + moduleName + ".cloudide"
    );
    ...
    zip(this.toZipFiles, zipPath, this.pluginRootFolder).catch((e) =>
      console.error(e)
    );
  }
