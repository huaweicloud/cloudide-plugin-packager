...
program
    .description(`A tool that help you personalize packaging, you can also create a 'pack-config.json' file 
directly to set files you want to include or exclude like: { "exclude": [], "include": [] }.`)
    ...
    .option('-e, --exclude-file <file>', 'Input file to exclude, or use batch matching with quotation marks.',
        (file: string, previous: string[]) => {
            return previous.concat(file);
        }, [])
    .option('-i, --include-file <file>', `Input file to include, or use batch matching with quotation marks, match rule like:
